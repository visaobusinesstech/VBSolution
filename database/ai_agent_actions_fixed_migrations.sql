-- Migrações corrigidas para ai_agent_actions
-- Este arquivo pode ser executado independentemente das outras tabelas
...
-- Função para obter estatísticas das ações
CREATE OR REPLACE FUNCTION get_ai_agent_action_stats(p_config_id UUID)
RETURNS JSONB AS $$
DECLARE
    stats JSONB;
BEGIN
    SELECT jsonb_build_object(
        'total_actions', COUNT(*),
        'active_actions', COUNT(*) FILTER (WHERE is_active = true),
        'inactive_actions', COUNT(*) FILTER (WHERE is_active = false),
        'total_executions', COALESCE(SUM((performance_metrics->>'execution_count')::INTEGER), 0),
        'average_success_rate', COALESCE(AVG((performance_metrics->>'success_rate')::FLOAT), 0.0),
        'most_used_action', (
            SELECT action 
            FROM ai_agent_actions 
            WHERE ai_agent_config_id = p_config_id 
