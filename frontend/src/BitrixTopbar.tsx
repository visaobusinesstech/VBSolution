...
export function BitrixTopbar({
  ...
  const handleSearchSubmit = () => {
    ...
    if (!searchText) {
      return; // Não faz nada se não há texto
    }
    ...
    if (foundPage) {
      ...
      navigate(foundPage);
      setSearchTerm(''); // Limpa o campo após redirecionamento
      return;
    }
    
    // Se não encontrou página pela palavra-chave, faz busca contextual
    ...
    setSearchTerm(''); // Limpa o campo após busca
  };
  ...
  return (
    <header 
      className="fixed top-0 left-0 right-0 h-[38px] border-b shadow-lg z-30 transition-colors duration-200"
      style={{ 
              ...
              <button className="flex items-center gap-2 p-1 rounded-lg hover:bg-blue-800/30 transition-colors">
                <Avatar className="h-6 w-6 border-2 border-white/20">
                  ...
                  <AvatarFallback className="bg-blue-600 text-white text-xs font-medium">
                    {(profile.name || user?.user_metadata?.name)?.substring(0, 2).toUpperCase() || 'U'}
                  </AvatarFallback>
                </Avatar>
              </button>
            </DropdownMenuTrigger>
