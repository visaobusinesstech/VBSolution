...
# Inserir conversas recentes
echo "📱 Inserindo conversa 1 - Cliente Ativo..."
...
echo "📱 Inserindo conversa 2 - Suporte Técnico..."
curl -X POST "${SUPABASE_URL}/rest/v1/whatsapp_atendimentos" \
  "${HEADERS[@]}" \
  -d "{
    \"owner_id\": \"00000000-0000-0000-0000-000000000000\",
...
echo "📱 Inserindo conversa 3 - Vendas..."
curl -X POST "${SUPABASE_URL}/rest/v1/whatsapp_atendimentos" \
  "${HEADERS[@]}" \
  -d "{
    \"owner_id\": \"00000000-0000-0000-0000-000000000000\",
...
echo "📱 Inserindo conversa 4 - Finalizada..."
curl -X POST "${SUPABASE_URL}/rest/v1/whatsapp_atendimentos" \
  "${HEADERS[@]}" \
  -d "{
    \"owner_id\": \"00000000-0000-0000-0000-000000000000\",
...
# Aguardar um pouco para os atendimentos serem criados
sleep 2

# Obter IDs dos atendimentos criados
ATENDIMENTO_1=$(curl -s "${SUPABASE_URL}/rest/v1/whatsapp_atendimentos?numero_cliente=eq.5511999887766&select=id" \
  "${HEADERS[@]}" | jq -r '.[0].id')

ATENDIMENTO_2=$(curl -s "${SUPABASE_URL}/rest/v1/whatsapp_atendimentos?numero_cliente=eq.5511888776655&select=id" \
  "${HEADERS[@]}" | jq -r '.[0].id')

ATENDIMENTO_3=$(curl -s "${SUPABASE_URL}/rest/v1/whatsapp_atendimentos?numero_cliente=eq.5511777665544&select=id" \
  "${HEADERS[@]}" | jq -r '.[0].id')

ATENDIMENTO_4=$(curl -s "${SUPABASE_URL}/rest/v1/whatsapp_atendimentos?numero_cliente=eq.5511666554433&select=id" \
  "${HEADERS[@]}" | jq -r '.[0].id')

# Inserir mensagens para cada conversa
if [ "$ATENDIMENTO_1" != "null" ] && [ "$ATENDIMENTO_1" != "" ]; then
  echo "💬 Adicionando mensagens para Maria Silva..."
  curl -X POST "${SUPABASE_URL}/rest/v1/whatsapp_mensagens" \
    "${HEADERS[@]}" \
    -d "{
      \"owner_id\": \"00000000-0000-0000-0000-000000000000\",
      \"atendimento_id\": \"$ATENDIMENTO_1\",
...
if [ "$ATENDIMENTO_2" != "null" ] && [ "$ATENDIMENTO_2" != "" ]; then
  echo "💬 Adicionando mensagens para João Santos..."
  curl -X POST "${SUPABASE_URL}/rest/v1/whatsapp_mensagens" \
    "${HEADERS[@]}" \
    -d "{
      \"owner_id\": \"00000000-0000-0000-0000-000000000000\",
      \"atendimento_id\": \"$ATENDIMENTO_2\",
...
if [ "$ATENDIMENTO_3" != "null" ] && [ "$ATENDIMENTO_3" != "" ]; then
  echo "💬 Adicionando mensagens para Ana Costa..."
  curl -X POST "${SUPABASE_URL}/rest/v1/whatsapp_mensagens" \
    "${HEADERS[@]}" \
    -d "{
      \"owner_id\": \"00000000-0000-0000-0000-000000000000\",
      \"atendimento_id\": \"$ATENDIMENTO_3\",
...
if [ "$ATENDIMENTO_4" != "null" ] && [ "$ATENDIMENTO_4" != "" ]; then
  echo "💬 Adicionando mensagens para Carlos Oliveira..."
  curl -X POST "${SUPABASE_URL}/rest/v1/whatsapp_mensagens" \
    "${HEADERS[@]}" \
    -d "{
      \"owner_id\": \"00000000-0000-0000-0000-000000000000\",
      \"atendimento_id\": \"$ATENDIMENTO_4\",
